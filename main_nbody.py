...
def make_pyg_loader(dataset, batch_size, shuffle, num_workers, loop=True, dense_mode=True):
    data_list = []
    radius_graph = RadiusGraph(args.radius, loop=loop, max_num_neighbors=1000)
    for data in dataset:
        ...
        if (not dense_mode) or (not loop):  # Because if loop=False we need the graph approach
